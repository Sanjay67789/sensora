...
class I2CScanner(BaseScanner):
    """
    Discover devices connected to Linux I²C adapters.
    """
    ...
    def scan(self) -> Result:
        ...
        adapters = sorted(
            self._device_directory.glob("i2c-*")
        )
        ...
        for adapter in adapters:
            ...
            try:
                ...
                for address in addresses:
                    ...
                    device = self._matcher.match_i2c(
                        address
                    )

                    if device is None:
                        device = (
                            self._matcher.create_unknown_i2c(
                                address
                            )
                        )
        ...
        return Result(
            success=True,
            message=(
                f"Scanned {scanned_buses} bus(es), "
